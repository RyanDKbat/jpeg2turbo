...
int main()
{
    ...
    FILE* fw_red = fopen("/home/nvidia/project/red_data.txt", "r");
    for (j = 0; j < pic_size; j++)
        fscanf(fw_red, "%d", &dataA[j]);
    fclose(fw_red);           //读取red
    FILE* fw_green = fopen("/home/nvidia/project/green_data.txt", "r");
    for (j = 0; j < pic_size; j++)
        fscanf(fw_green, "%d", &dataB[j]);//读取green
    fclose(fw_green);
    FILE* fw_blue = fopen("/home/nvidia/project/blue_data.txt", "r");
    for (j = 0; j < 250000; j++)
   ...
   clock_t end = clock();
    ofstream out_red("/home/nvidia/project/transport/red_encode.txt");
    ...
    out_red.close();
    ofstream out_green("/home/nvidia/project/transport/green_encode.txt");
    for (int k = 0; k < 250000 * 16; k++)//rgb编码输出
    {
    ...
    out_green.close();
    ofstream out_blue("/home/nvidia/project/transport/blue_encode.txt");
    for (int k = 0; k < 250000 * 16; k++)//rgb编码输出
    {
