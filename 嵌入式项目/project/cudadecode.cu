...
double __device__    mk(double a, double s, double p,int c, int u)  //Mk(e)的计算//
{
    double mk;
    if (u == 1)
        mk = a - log(1 + myexp(a)) + 1 / 2 * s + 1 / 2 * p * (2 *c - 1);
    else
        mk = -log(1 + myexp(a)) - 1 / 2 * s + 1 / 2 * p * (2 * c - 1);
    return(mk);
}
...
//分量译码器//
void __device__   DEC(double a[SIZE + 1], double ys[SIZE + 1], double yp[SIZE + 1], double e[SIZE + 1])
{
...
{
    double x[SIZE + 1][SIZE + 1], y[SIZE + 1][3], y0[SIZE + 1], y1[SIZE + 1], y2[SIZE + 1];
    ...
    if (p < 250000)//线程量约束，可根据数据量大小更改
    {
        ...
        for (i = 15; i >= 0; i--)
        {
            data1[i] = dataA[p] - floor(dataA[p] / 10) * 10;
            dataA[p] /= 10;
        }
        for (i = 1; i <= SIZE; i++)
            for (j = 1; j < 3; j++)
...
int main()
{
	...
	ifstream infile;
	infile.open("/home/nvidia/project/transport/red_encode.txt");
    for (j = 0; j < 250000; j++)
      { infile>>dataA[j];}
    infile.close();
    //fclose(fw_red);           //读取red
    //FILE* fw_green = fopen("/home/nvidia/project/transport/green_encode.txt", "r");
    infile.open("/home/nvidia/project/transport/green_encode.txt");
    for (i = 0; i < 250000; i++)
       { infile>>dataB[i];}//读取green
	infile.close();
    //fclose(fw_green);
   // FILE* fw_blue = fopen("/home/nvidia/project/transport/blue_encode.txt", "r");
   infile.open("/home/nvidia/project/transport/blue_encode.txt");
    for (n = 0; n < 250000; n++)
       { infile>>dataC[n];}//读取blue
	infile.close();
    //fclose(fw_blue);
    clock_t start = clock();
